...
@inline function aux_state_init!(aux, x, y, z)
  @inbounds begin
    aux[_a_x] = x
    aux[_a_y] = y
    aux[_a_z] = z
...
@inline function bcstate!(QP, VFP, auxP, nM, QM, VFM, auxM, bctype, t, PM, uM, vM, wM, ρinv)
  @inbounds begin
...
function run(mpicomm, dim, topl, N, timeend, DFloat, dt)
  ...
  spacedisc = DGBalanceLaw(grid = grid,
                           ...
                           flux! = cns_flux!, # Physical flux function 
                           numerical_flux! = numflux!, # Numerical flux function 
                           numerical_boundary_flux! = numbcflux!, # Numerical boundary flux function 
                           number_gradient_states = _ngradstates, # Number of gradient states
...
let
  ...
  brickrange = (range(DFloat(0); length=Ne[1]+1, stop=1000),
                ...
                range(DFloat(0); length=Ne[2]+1, stop=1000))
  topl = BrickTopology(mpicomm, brickrange, periodicity = (true, false, false))
  dt = 1e-2 / Ne[1]
  timeend = 1.0
  result[l] = run(mpicomm, dim, topl, polynomialorder, timeend,
                  DFloat, dt)
end
