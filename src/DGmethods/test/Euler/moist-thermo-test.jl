...
@inline function preflux(Q, _...)
  ...
  e_int = internal_energy(T_0)
  q_tot = 0.00196
  ρ = 1.22
  ts = PhaseEquil(e_int, q_tot, ρ)
  v = soundspeed_air(ts)
  v = gas_constant_air(ts)
  v = air_pressure(ts)
  v = air_density(ts)
  v = cp_m(ts)
  v = cv_m(ts)
  v = moist_gas_constants(ts)
  v = air_temperature(ts)
  v = internal_energy_sat(ts)
  v = latent_heat_vapor(ts)
  v = latent_heat_sublim(ts)
  v = latent_heat_fusion(ts)
  v = saturation_shum(ts)
  v = saturation_excess(ts)
  v = liquid_fraction_equil(ts)
  v = liquid_fraction_nonequil(ts)
  v = PhasePartition(ts)
  v = liquid_ice_pottemp(ts)
  v = dry_pottemp(ts)
  v = exner(ts)
  v = liquid_ice_pottemp_sat(ts)
  v = specific_volume(ts)
  v = virtual_pottemp(ts) 
end
