...
const _nauxstate = 12
const _a_ϕ, _a_ϕx, _a_ϕy, _a_ϕz, _a_ρ_ref, _a_ρe_ref, _a_x, _a_y, _a_z, _a_sponge, _a_Δh, _a_Δv= 1:_nauxstate
...
function eulerflux!(F, Q, VF, aux, t)
  @inbounds begin
    ...
    Δh = aux[_a_Δh]
    Δv = aux[_a_Δv]
    (ν_1, ν_2, ν_3, _,_,_) = anisotropic_smagorinsky(SijSij, Δh, Δh, Δv)
    f_R = 1
    ...
    F[1, _dρe] -= u * τ11 + v * τ12 + w * τ13 + ν_1 * k_μ * vTx
    F[2, _dρe] -= u * τ21 + v * τ22 + w * τ23 + ν_2 * k_μ * vTy
    F[3, _dρe] -= u * τ31 + v * τ32 + w * τ33 + ν_3 * k_μ * vTz
  end
end
#md nothing # hide
...
function auxiliary_state_initialization!(Ne_vertical, Ne_horizontal, T0, domain_height, aux, x, y, z, MTS)
  @inbounds begin
    DFloat = eltype(aux)
    p0 :: DFloat = MSLP
    θ0 :: DFloat = 315
    ...
    aux[_a_y] = y
    aux[_a_z] = z
    aux[_a_Δh] = 2*pi*h/Ne_horizontal
    aux[_a_Δv] = domain_height/Ne_vertical
...
function setupDG(mpicomm, Ne_vertical, Ne_horizontal, polynomialorder,
  ...
  auxinit!(x...) = auxiliary_state_initialization!(Ne_horizontal, Ne_vertical,T0, domain_height, x...)
...
#  filter = Grids.CutoffFilter(spatialdiscretization.grid, 5)
  filter = Grids.ExponentialFilter(spatialdiscretization.grid, 0, 16)
