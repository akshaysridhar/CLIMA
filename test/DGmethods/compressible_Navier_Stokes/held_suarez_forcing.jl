...
const _nauxstate = 14
const _a_ϕ, _a_ϕx, _a_ϕy, _a_ϕz, _a_ρ_ref, _a_ρe_ref, _a_x, _a_y, _a_z, _a_sponge, _a_Δ1, _a_Δ2, _a_Δ3, _a_Δsqr = 1:_nauxstate
...
function compute_stresses!(VF, grad_vars, _...)
  ...
  @inbounds begin
    ...
    # strains
    # --------------------------------------------
    (S11,S22,S33,S12,S13,S23,SijSij) = strainrate_tensor_components(dudx, dudy,
                                                                   dudz, dvdx,
                                                                   dvdy, dvdz,
                                                                   dwdx, dwdy,
                                                                   dwdz)
...
function eulerflux!(F, Q, VF, aux, t)
  @inbounds begin
    ...
    (ν_1, ν_2, ν_3, D_1, D_2, D_3) = (100,100,0,0,0,0) #2D viscosity only
    f_R = 1
...
function auxiliary_state_initialization!(T0, domain_height, aux, x, y, z, MTS)
  @inbounds begin
    DFloat = eltype(aux)
    p0 :: DFloat = MSLP
    θ0 :: DFloat = 315
    ...
    dx = abs(1/2hypot(ξx,ηx,ζx))
    dy = abs(1/2hypot(ξy,ηy,ζy))
    dz = abs(1/2ζz)
    
    (dr, dλ, dϕ) = cartesian_to_spherical(dx, dy, dz)
    aux[_a_Δ1] = 0*1e6 #dx
    aux[_a_Δ2] = 0*1e6 #dy
    aux[_a_Δ3] = 0*1e6 #dz
    aux[_a_Δsqr] = 1e10 #dz
...
#  outputtime =1*days
  ...
  function do_output(vtk_step)
    ...
    if MPI.Comm_rank(mpicomm) == 0
      ...
      pvtuprefix = @sprintf("%s/held_suarez_forcing_step%04d", VTKDIR, vtk_step)
