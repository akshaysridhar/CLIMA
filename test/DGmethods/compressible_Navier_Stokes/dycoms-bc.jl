...
@inline function bcstate!(QP, VFP, auxP, nM, QM, VFM, auxM, bctype, t)
  @inbounds begin
    ...
    if auxM[_a_z] < 0.001 
      ...
      # ------------------------------------
      q_vap_star = q_vap_saturation(SST, ρM, PhasePartition(q_totM, q_liqM, 0.0))
      ...
      # ---------------------------------------
      # cpm_FN           = cp_m(TS_FN)
      # SHF      = - Cd * windspeed_FN * (cpm_FN*T_FN - cpm*SST + grav * (z_FN - zmin)) 
      
      # ----------------------------------------------
      # Assigning calculated values to boundary states
      # ----------------------------------------------
      Flux_prescribed = 130 #[W/m²]
      VFP[_τ33] = 0  # Case specific for flat bottom topography, normal vector is n⃗ = k⃗ = [0, 0, 1]ᵀ
      VFP[_τ13] = -ρM * Cd * windspeed_FN * u_FN 
      VFP[_τ23] = -ρM * Cd * windspeed_FN * v_FN 
      VFP[_qz]  = 115 /(ρM * LH_v0) #-ρ * Cd * windspeed_FN * (q_vap_FN - q_vap_star)
      VFP[_JplusDz] = 130
    end
    nothing
  end
