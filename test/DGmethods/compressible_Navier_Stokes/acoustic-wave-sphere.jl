...
const Δ = 100  # FIXME 
const Δsqr = Δ * Δ #anisotropic_coefficient_sgs(Δx, Δy, Δz, Npoly)
#Earth Constants
...
@inline function compute_stresses!(VF, grad_vel, _...)
  ...
  @inbounds begin
    ...
    # Smagorinsky closure
    (ϵ11,ϵ22,ϵ33,ϵ12,ϵ13,ϵ23, ϵijϵij) = compute_strainrate_tensor(dudx, dudy, dudz, 
                                                                  dvdx, dvdy, dvdz, 
                                                                  dwdx, dwdy, dwdz)

    (μ_e, D_e, ϵijϵij) = static_smag(dudx, dudy, dudz, 
                                      dvdx, dvdy, dvdz, 
                                      dwdx, dwdy, dwdz,
                                      Δsqr)

    # deviatoric stresses
    VF[_τ11] = 2μ_e * (ϵ11 - (ϵ11 + ϵ22 + ϵ33) / 3)
    VF[_τ22] = 2μ_e * (ϵ22 - (ϵ11 + ϵ22 + ϵ33) / 3)
    VF[_τ33] = 2μ_e * (ϵ33 - (ϵ11 + ϵ22 + ϵ33) / 3)
    VF[_τ12] = 2μ_e * ϵ12
    VF[_τ13] = 2μ_e * ϵ13
    VF[_τ23] = 2μ_e * ϵ23
  end
...
#    nsimstep=1000
    ...
    cbinfo = GenericCallbacks.EveryXSimulationSteps(nsimstep) do
      (δρ_max,δρ_min) = (0,0) #global_extrema_diff(Q,spacedisc.auxstate,_ρ)
        (δU_max,δU_min) = (0,0) #global_extrema_diff(Q,spacedisc.auxstate,_U)
        (δV_max,δV_min) = (0,0) #global_extrema_diff(Q,spacedisc.auxstate,_V)
        (δW_max,δW_min) = (0,0) #global_extrema_diff(Q,spacedisc.auxstate,_W)
        (δE_max,δE_min) = (0,0) #global_extrema_diff(Q,spacedisc.auxstate,_E)
        @info @sprintf """Update
        simtime = %.16e
        runtime = %s
        Δmass   = %.16e
    ...
    cbvtk = GenericCallbacks.EveryXSimulationSteps(nsimstepvtk) do (init=false)
        ...
        writevtk(outprefix, Q, spacedisc, statenames, spacedisc.auxstate, auxnames)
        step[1] += 1
        nothing
    end
    #------------Set Callback Info--------------------------------#
