...
@inline function bcstate!(QP, VFP, auxP, nM, QM, VFM, auxM, bctype, t)
    ...
    @inbounds begin
      ...
      elseif bctype == 3
        y = auxM[_a_y]
        VFP[_Ty] = 0  # dTdy = 0 at wall (bottom) 
        QP[_E] = EM
      elseif bctype == 4
        y = auxM[_a_y]
        VFP[_Ty] = 0  # dTdy = 0 at wall (top)
        QP[_E] = EM
      end
        nothing
    end
...
function dry_benchmark!(dim, Q, t, spl_tinit, spl_pinit, spl_thetainit, spl_qinit, x, y, z, _...)
    ...
    q_partition = PhasePartition(q_tot, 0.0, 0.0)
...
function run(mpicomm, dim, Ne, N, timeend, DFloat, dt)
  ...
  @timeit to "Topo init" topl = StackedBrickTopology(mpicomm, brickrange, periodicity=(false,false), boundary=[1 3;2 4])
  ...
  @timeit to "IC init" begin
    ...
    initialcondition(Q, x...) = dry_benchmark!(Val(dim), Q, DFloat(0), spl_tinit, spl_pinit, spl_thetainit, spl_qinit, x...)
  ...
  @timeit to "Time stepping init" begin
    ...
    cbvtk = GenericCallbacks.EveryXSimulationSteps(5000) do (init=false)
      ...
      mkpath("./Rayleigh-Benard/")
      outprefix = @sprintf("./Rayleigh-Benard/RB_%dD_mpirank%04d_step%04d", dim,
