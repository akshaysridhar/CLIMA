...
# Problem description 
# --------------------
# Baroclinic instability case. 3D box reproduction of 
# Jablonowski's test case for atmospheric dynamical cores
...
const (xmin, xmax) = (0, 4e7)
const (ymin, ymax) = (0, 6e6) 
const (zmin, zmax) = (0, 3e4)
...
function compute_baroclinic_instability(x, y, eta, f0, beta0, u0, T_0, γ_lapse, Ly)
  R_gas::eltype(x) = R_d 
  b = 2.0
  y0 = 0.5 * Ly 
  b2 = b * b 
  exp1 = R_gas * γ_lapse / grav
  Tmean = T_0 * eta^exp1
  phimean = T_0 * grav/ γ_lapse * (1-eta^exp1)
  logeta = log(eta)
  fac1 = (f0 - beta0*y0)*(y - 0.5*Ly - Ly/(2*π)*sinpi(2*y/Ly))
  fac2 = 0.5 * beta0 * (y^2 - Ly * y / π * sinpi(2*y/Ly) - 0.5 * (Ly / π)^2 * cospi(2 * y/ Ly) - Ly^2/3 - 0.5 * (Ly/π)^2)
  fac3 = exp(-logeta*logeta/b2)
  phi_prime = 0.5 * u0 * (fac1 + fac2) 
...
for the baroclinic instability test 
"""

function baroclinic_instability!(dim, Q, t, x, y, z, _...)
  DFloat         = eltype(Q)
  p0::DFloat     = MSLP
  R_gas::DFloat  = R_d
  
  # Baroclinic Instability in a periodic box test case
  up = 1.0
  Lp = 600000
  Lp2 = Lp*Lp
  xc = 2000e3
  ...
  eta = 1e-14
  for ii = 1:maxiter 
    geo_phi, temp = compute_baroclinic_instability(x, y, eta, f0, beta0, u0, T_0, γ_lapse, Ly)
    numerator = -grav * z + geo_phi
    denominator = -R_gas / eta / temp
    deta = numerator / denominator 
    eta = eta - deta
    if (abs(deta) <= 1e-14 ) break
    elseif ii == maxiter
      error("convergence failed : η coordinates in initial condition") 
    end
  end
  eta = min(eta, 1.0)
  eta = max(eta, 0.0)     
  ...
  T = temp 
  P = p0 * eta 
  θ = T * (p0 / pressure)^(R_gas/c_p)
  
  # Tref in NUMA line 5440
  θ_ref = T_0 * (1 - γ_lapse * z / T_0)^(- gravity / (cp * gamma_lapse))
  ρ_ref = p0 / (T_0 * R_gas) * (1 - γ_lapse * z/T_0)^(grav/(R_gas * γ_lapse) - 1)
...
# spacedisc = data needed for evaluating the right-hand side function
    ...
    initialcondition(Q, x...) = baroclinic_instability!(Val(dim), Q, DFloat(0), x...)
    ...
    cbvtk = GenericCallbacks.EveryXSimulationSteps(200) do (init=false)
        ...
        outprefix = @sprintf("vtk-baroclinic/cns_%dD_mpirank%04d_step%04d", dim,
