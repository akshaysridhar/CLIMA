...
const _nauxstate = 12
const _a_z, _a_dx, _a_dy, _a_dz, _a_sponge, _a_02z, _a_z2inf, _a_T, _a_P, _a_q_liq, _a_soundspeed_air,_a_timescale = 1:_nauxstate
...
function preodefun!(disc, Q, t)
  ...
  @inbounds let
    ...
    R[_a_P] = P
    R[_a_q_liq] = q_liq
    R[_a_soundspeed_air] = soundspeed
          ...
          DGBalanceLawDiscretizations.dof_iteration!(spacedisc.auxstate, spacedisc,
            Q) do R, Q, QV, aux
              @inbounds let
                ...
                TS = PhaseEquil(e_int, q_tot, ρ)
                soundspeed  = soundspeed_air(TS)
                u_timescale = (abs(u) + soundspeed) * (2*Npoly+1)/ dx
                v_timescale = (abs(v) + soundspeed) * (2*Npoly+1)/ dy 
                w_timescale = (abs(w) + soundspeed) * (2*Npoly+1)/ dz 
